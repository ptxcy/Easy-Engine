...
void main() {
    ...
    // Material
    vec4 baseColorTex = vec4(albedo, 1.0);
    vec3 baseColorRGB = albedo;

    if(useBaseColor) {
        baseColorTex = texture(baseColorTexture, fragTexCoords);
        baseColorRGB = baseColorTex.rgb;
    }
    ...
    vec3 F0 = mix(vec3(0.04), baseColorRGB, finalMetallic);
    ...
    vec3 Lo = (kD * baseColorRGB / 3.14159265 + specular) * radiance * NdotL;
    vec3 ambient = vec3(0.03) * baseColorRGB * ao;
    vec3 color = ambient + Lo;

    color = pow(color, vec3(1.0/2.2));
    resultColor = vec4(color, baseColorTex.a);
}
